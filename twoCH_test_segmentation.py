...
from twoCH_dataset_aug import twoCH
...
def test(test_data_path, model_file_name, output_path,
    ...
    # Create dataset
    test_dataset = twoCH(test_data_path)
    ...
    result_file_name = f'{output_path}/test_result_th{binarize_threshold:.3f}_{time_stamp}_2ch.csv'

    model.load_state_dict(torch.load(model_file_name, map_location=device))
    model = model.to(device)
    model.eval()

    dice_coeff_arr = np.zeros((2,test_dataset.__len__()))
    ...
    with torch.no_grad():
        ...
        for batch_idx, (org_data, org_labels) in enumerate(test_loader):
            ...
            for i in range(outputs.shape[1]):
                ...
                if export_mask:
                    mask_file_name = f'{output_path}/test_images/mask_th{binarize_threshold:.3f}_{batch_idx:05d}_ch{i}.png'
                    output = overlay(out_mask,label_img,org_data)
                    imageio.imwrite(mask_file_name, output)

                print(f"{batch_idx},ch{i},{dice_coeff_arr[i][batch_idx]:.4f}")
